...
class MinimaxPlayer(IsolationPlayer):
    ...
    def time_check(self):
        # Adding a buffer of .02 seconds to aide in removing the timeout exceptions
        if self.time_left() <= self.TIMER_THRESHOLD - .02:
            raise SearchTimeout()
    ...
    def max_value(self, game, search_depth):
        ...
        for move in game.get_legal_moves():
            ...
            v = max(v, self.min_value(forecast, search_depth - 1))
        return v
    ...
    def min_value(self, game, search_depth):
        ...
        for move in game.get_legal_moves():
            forecast = game.forecast_move(move)
            v = min(v, self.max_value(forecast, search_depth - 1))
        return v
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        try:
            search_depth = 1
            while True:
                current_move = self.alphabeta(game, search_depth)
                if current_move == (-1, -1):
                    return best_move
                else:
